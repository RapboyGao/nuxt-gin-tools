...
export const PACKAGE_JSON_CONTENT = {
  private: true,
  scripts: {
    start: "./.build/.server/production.exe", // 定义启动命令
  },
};
...
export const FILES_TO_COPY = {
  ...
  ".build/.server/production.exe": builtPath(".build/.server/production.exe"), // 生产环境可执行文件
...
function writeScriptFiles() {
  ...
  // 写入 PowerShell 启动脚本
  FS.outputFileSync(builtPath("start.ps1"), `./.build/.server/production.exe`);
  // 写入 Linux/macOS 启动脚本
  FS.outputFileSync(builtPath("start.sh"), `./.build/.server/production.exe`);
